...
public class Game {
    ...
    public Game(){
        ...
        // Säg att du ska köra "run" i Person klassen efter 5 sekunder en gång
//        pool.schedule(Jason, 2, TimeUnit.SECONDS);
//        pool.schedule(Freddy, 2, TimeUnit.SECONDS);
//        pool.schedule(TureSventon, 2, TimeUnit.SECONDS);

        Jason.scheduleAction();
        Freddy.scheduleAction();
        TureSventon.scheduleAction();
        ...
        while (true) {
            ...
            if (!command.equals("-1")) {
                ...
                if (command.contains("add")){
                    ...
                    if (command.contains("old Shoes")){
                        inventory.addObject(Shoes);
                    }
                    if (command.contains("Rock")){
                        inventory.addObject(Rock);
                    }
                }
